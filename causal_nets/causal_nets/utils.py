...
class CoeffNet():
    ...
    def _building_the_model(self, nfeatures):
        ...
        if self.optimizer == 'Adam':
            opt = keras.optimizers.Adam(learning_rate=self.learning_rate, beta_1=0.9,
                                        beta_2=0.999, epsilon=None, decay=0.0,
                                        amsgrad=True)
        elif self.optimizer == 'GradientDescent':
            opt = keras.optimizers.SGD(learning_rate=self.learning_rate, momentum=0.0,
                                       decay=0.0, nesterov=False)
        else:
            opt = keras.optimizers.RMSprop(learning_rate=self.learning_rate, rho=0.9,
