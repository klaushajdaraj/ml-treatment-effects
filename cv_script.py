...
def save_params(name: str, params: dict):
    filename = params_file_path + name + ".txt"
...
cf_grid = [
    ...
    for max_features in [i / 10.0 for i in range(2, 11)]
    for max_samples in [i / 10.0 for i in range(1, 6)]
    for min_samples_leaf in [5, 10, 20, 50]
    for min_var_fraction_leaf in [None, 0.1, 0.2, 0.3, 0.4]
